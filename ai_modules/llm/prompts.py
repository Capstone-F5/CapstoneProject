...
- 이미 장바구니에 있는 항목의 제외 옵션이나 세트 옵션을 변경 요청하면:
  remove_item으로 기존 항목 삭제 후 add_item으로 새 옵션으로 재담기.
  이때 기존 수량과 다른 옵션은 그대로 유지한다.
  예: 치즈버거 2개(양파 없이) 요청 → remove_item(5) → add_item(5, quantity=2, exclusion=양파 제외)
  update_qty는 수량 변경 전용이며 옵션 변경에 사용 금지.

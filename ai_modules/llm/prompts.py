...
- "추천 메뉴 알려줘" / "인기 메뉴 뭐야" / "뭐가 맛있어?" → navigate('menu') → select_category(recommended) →
  ...
  [추천메뉴] 표시가 없는 항목은 언급하지 않는다. list_menu를 호출하지 않고 짐작으로 답하지 않는다.
...
## 메뉴 추천 시나리오
- '뭐가 맛있어요?', '인기 메뉴 뭐예요?' → list_menu를 호출해 "[추천메뉴]" 표시가 붙은 항목만 안내한다.
  표시가 없는 항목은 언급하지 않으며, 도구를 호출하지 않고 짐작으로 답하지 않는다.
